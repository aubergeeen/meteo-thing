from django.db.models import Avg, Max, Min, FloatField, Sum
...
# Подразумевается, что вызывается только из object manager'а нужной модели??
...
# Подсчитывает агрегат (1!) для заданного параметра (для ВСЕХ станций)
""" 
def aggregate_per_station(queryset, param_type, aggregate_func='avg', start, end):
    # TO-DO проверка, что queryset принадлежит той модели которая подразумевается
    # получаем Trunc и агрегат. ф-ии в соответсвии с переданым параметром
